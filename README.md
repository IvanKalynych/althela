...
3. ADMIN у адмін-панелі:
   ...
   b. Обирає тип контракту (впливає на логіку оплати при бронюванні):
      ...
      - Тип 2: Часткова передоплата (50% з кредитів організації, 50% доплачує клієнт)
      - Тип 3: Дисконтна модель (безоплатний для компанії, 90% платить клієнт, 10% бере платформа як субсидію)
...
CORPORATE CLIENT → Обирає терапевта → Обирає вид консультації (відео / чат)
  ...
  [Тип 1 — 100% передоплата]:
    ...
    → [ENOUGH CREDITS] → Кредити списуються в момент бронювання, бо ми працюємо по передплаті, сесія може не відбутись взагалі, але якщо воне не відбулась бо клієнт не з"явився гроші не повертаються.
...
> **Для терапевта участь у корпоративній програмі означає:** -10% до доходу від корпоративних сесій, але стабільний потік клієнтів. Різниця між корпоративним і звичайним клієнтом для терапевта не відображається в інтерфейсі, окрім позначення що це корпоративний клієн — він обслуговує всіх однаково.
...
| Плата за скасування | 0% (>48h), 50% (<24h), 100% (<6h) |
| Платіжний процесор | Stripe + альтернативи (LiqPay, WayForPay для локальних оплат, SWIFT для B2B) |
...
| Компонент | Деталі |
|---|---|
| Типи контрактів | Тип 1: 100% передоплата · Тип 2: Часткова передоплата · Тип 3: безоплатний |
| Кредитна система | 1 EUR = 1 кредит; кредити зберігаються на рахунку організації |
| Оплата | Залежить від типу контракту (Для договору 50на50 система така 50 платить компанія Альфа на рахунок компанії Альтена (це не страйп це звичайна оплата інвойсу наприклад свіфтом), Для договору "Дисконт" - платить тільки фізособа через страйп просто вона сплачує 90% вартості, а 10% вартості Альтена бере собі умовно кажучи як збиток, бо в такому випадку Альтена отримає на 20% від сесії а лише 10%.) |
...
| Комісія платформи | контракт 100% предплата, комісія платформи з кожної сесії -30%, контракт 50 на 50, комісія платформи аналогічно - 30 % але з двох частин складається, 50 прийде на рахунок Альтени від Альфи, а інші 50 прийдуть від клієнта через страйп на рахунок Альтени. 2 рази на місяць Альтена робить звіт по контрактам і проводить відповідну оплату консультантам (70%).
і тільки в третьому випадку договір -"дисконт" комісія платформи 20%. |
...
enum ContractType {
  ...
  postpay          // Тип 3: Дисконтна модель (безоплатний для компанії, 90% платить клієнт, 10% бере платформа як субсидію)
}
...
> **Примітка:** Кабінет корпоративного клієнта ідентичний звичайному клієнту. Єдине додаткове — секція в Settings для введення коду контракту та footer-рядок з інформацією про організацію. Код контракту вводиться при реєстрації, а в налаштуваннях його можна змінити при заміні.
...
| Щотижнева виплата терапевту В2С | Cron: пн 09:00 | Зведення виплат |
| Кожні 2 тижні виплата терапевту В2B | Cron: пн 09:00 | Зведення виплат |
| Генерація рахунку B2B (Тип 3) | Cron: 1-го числа 06:00 | Stripe Invoice для postpay організацій, тільки для контрактів типу A,B для контракту типу С - рахунок не передбачено |
...
- Статистика: кількість active corporate consultants, покриття по регіонах/спеціалізаціях - покриття не по регіонах, спеціалізаціях а покриття потреб бізнесу у часах. Тобто спеціалісти самостійно вказують час своєї доступності, і ми не знаємо якого спеціаліста обере клієнт.  Важливо розуміти перше-чи достатньо у нас спеціалістів, чи треба активувати рекламну компанію щодо співробітництва та корпоративного обслуговування. Друге - чи не "відпадають" клієнти через те що не знаходять слоти в бажаний для себе час у спеціаліста. Ну і взагалом. Потрібна статистика реакції клієнтів на платформі щоб розуміти "час пік" і "Мертвий сезон" і оперативно реагувати. Наприклад Запропонувати загальну знижку на консультацію будь якого спеціаліста в "мервий сезон" будь який прибуток - буде краще ніж ніякого.
...
7. **Рахунки** — для типів A, B: список Stripe Invoice
